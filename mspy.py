...
def read_snap(dir, snap):
        ...
        pars = np.loadtxt(dir + 'SCFPAR', dtype=str, usecols=(0,1))
        ...
        tsnap = float(np.loadtxt(dir+snapf, usecols=(0,1),dtype=str)[0,1])
...
class gen_ms ():
        ...
        def get_stats_fobs ( self, box_para=10., box_perp=80., nbins=31, **kwargs ):
                # XXX
                print('A')
                ...
                mag_asym_fo = []
                mu_ratio = []
                for i in np.arange(self.rpx.size):
                        x,y = gen_test_set ( self, i, box_para=box_para, box_perp=box_perp,
                                             **kwargs )
                        rebase = y * np.pi / y.max()
                        bins = np.linspace(-box_perp//2, box_perp//2, nbins)
                        stat,edges,_=stats.binned_statistic(rebase, np.ones_like(rebase),
                                                            'count', bins=bins)
                        bins = (edges[1:]+edges[:-1])/2.

                        fft = np.fft.rfft ( stat )
                        fft_nomean = fft
                        fft_nomean[0]=0
                        fft_sym = np.sum(abs(np.fft.irfft(np.real(fft_nomean))))
                        fft_asym = np.sum(abs(np.fft.irfft(np.imag(fft_nomean))))

                        mag_sym_fo.append( fft_sym )
                        mag_asym_fo.append ( fft_asym )
                        mu_ratio_fo.append ( fft_sym / (fft_sym + fft_asym) )
                self.mu_ratio_fo = np.asarray(mu_ratio)
                self.mag_sym_fo = np.asarray(mag_sym_fo)
                self.mag_asym_fo = np.asarray(mag_asym_fo)                        


        def get_stats_orth(self, nterms=5, **kwargs):
                print('a')
                n = len(self.rpx)
                indexes = []
                mag_sym = []
                ...
                for i in np.arange(n):
                        ...
                        try: x, y = gen_test_set(self,i, **kwargs)
                        except: continue
...
class sim_ms(particles):
        ...
        def get_stats_fourier(self, max_term=5, **kwargs):
                n = len(self.rpx)
                indexes = []
                sigma_imag = []
                sigma_real = []
        ...
        def plot_xy(self):
                ...
                if (hasattr(self, 'rpx') & hasattr(self, 'mu_ratio')):
                        plt.scatter(self.rpx, self.rpy, 
                                c=np.abs(self.mu_ratio), cmap=cmap, alpha=1,
                                    edgecolor='none', s=15, vmin=0., vmax=0.5)
                        plt.colorbar()
                if (hasattr(self, 'rpx')):
                        plt.scatter(self.rpx, self.rpy, 
                                c='cyan', alpha=1,edgecolor='none', s=5)
...
def plot_subset(sim_ms, k, box_perp = 20., box_para =20., alpha=0.05, nbins=10):
        #plot data used for a given ridge point
        plt.figure ( figsize=(12,4))
        plt.subplot(1,3,1)
        dx, dy = gen_test_set(sim_ms, k, box_perp = box_perp, box_para = box_para)
        bdx,bdy = gen_test_set(sim_ms, k, box_perp = 100, box_para=90 )#box_perp*2, box_para = box_para*10)
        #plt.scatter(sim_ms.x - sim_ms.rpx[k],
        #            sim_ms.y - sim_ms.rpy[k],
        #            s=2,c='k',alpha=0.03)
        plt.scatter(bdy,bdx, c='k', s=1, alpha=alpha, edgecolor='none')
        plt.scatter(dy, dx, c='r', s=1, alpha=alpha, edgecolor='none')
        #plt.ylim(dx.min() - 10., dx.max() + 10.)
        #plt.xlim(dy.min() - 10., dy.max() + 10.)

        plt.xlabel('y')
        plt.ylabel('x')
        
        plt.subplot(1,3,2)
        plt.scatter ( sim_ms.x, sim_ms.y, color='k', alpha=alpha/30, s=1)
        plt.scatter ( sim_ms.rpx[k], sim_ms.rpy[k], color='r', s=35 )
        
        plt.subplot(1,3,3)
        plt.hist(dy, bins=nbins, histtype='step', lw=3)
...
def get_single_morph_ortho(sim_ms, k, box_perp = 8., box_para = 4., max_term=100):
        ...
        rebase = y*(np.pi/y.max())#(2*np.pi/8.)
        n = float(len(y))
...
def plot_subset_fourier_gloess(sim_ms, k):
        ...
        #plot data used for a given ridge point
        plt.subplot(2,2,1)#, aspect='equal')
        dx, dy, rot = gen_test_set(sim_ms, k,rot_out=True, box_para=10, box_perp=80)
        plt.scatter(dy,dx, c='k', s=3, alpha=0.5, edgecolor='none')
        (ridgex, ridgey) = np.dot(rot, ((sim_ms.rpx, sim_ms.rpy)))
        plt.scatter(ridgey-ridgey[k],ridgex-ridgex[k],c='r', edgecolor='none')
        plt.xlim([-40,40])
